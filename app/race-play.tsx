    ...
    return (
      <ScreenShell texture={false}>
        <View style={styles.loading}>
          ...
          <RacePulse />
          <Text style={[styles.loadingTitle, { color: theme.colors.text, fontFamily: theme.typography.families.displayHeavy }]}>Getting both courses ready</Text>
          <Text style={[styles.loadingBody, { color: theme.colors.muted, fontFamily: theme.typography.families.body }]}>{mode === 'live' ? 'Waiting for the host to start the race…' : 'Building a fair practice opponent…'}</Text>
    ...
    return (
      <ScreenShell texture={false}>
        <View style={styles.finishedWait}>
          ...
          <Text style={[styles.finishedKicker, { color: accent, fontFamily: theme.typography.families.bodyMedium }]}>{first ? 'Your team finished first' : 'Your team finished'}</Text>
          <Text style={[styles.finishedTitle, { color: theme.colors.text, fontFamily: theme.typography.families.displayHeavy }]}>{view.phase === 'complete' ? 'Race complete' : 'Waiting at the finish'}</Text>
          <Text style={[styles.finishedBody, { color: theme.colors.muted, fontFamily: theme.typography.families.body }]}>{view.phase === 'complete' ? 'Checking the host timestamps…' : 'The other team is still solving its course.'}</Text>
    ...
    <ScreenShell edgeWire="none" padded={false} texture={false}>
      <View style={[styles.topbar, { borderBottomColor: theme.colors.draft }]}> 
        ...
        <View style={styles.teamIdentity}>
          ...
          <View>
            <Text style={[styles.teamLabel, { color: accent, fontFamily: theme.typography.families.bodyMedium }]}>{view.localTeamId === 'mint' ? 'Mint team' : 'Coral team'}</Text>
      ...
      <ScrollView contentContainerStyle={[styles.page, houseLineEnabled && styles.pageWithDock]} showsVerticalScrollIndicator={false}>
        {stage ? (
          <Animated.View entering={settings.reducedMotion ? undefined : SlideInRight.duration(320)} key={stage.id} style={styles.stage}>
            ...
            <View style={[styles.instructionBand, { backgroundColor: `${accent}18`, borderColor: accent }]}>
              <Ionicons color={accent} name="navigate-circle-outline" size={21} />
              <Text style={[styles.instruction, { color: theme.colors.text, fontFamily: theme.typography.families.body }]}>{stage.instruction}</Text>
            </View>
...
const styles = StyleSheet.create({
  accepting: { fontSize: 12, lineHeight: 16, textAlign: 'center' },
  ...
  countdownKicker: { color: '#17302B', fontSize: 14, lineHeight: 18 },
  countdownNumber: { color: '#07100E', fontSize: 170, lineHeight: 170 },
  countdownText: { color: '#07100E', fontSize: 26, lineHeight: 29 },
  disabled: { opacity: 0.36 },
  errorBand: { alignItems: 'center', borderRadius: 13, borderWidth: 1, flexDirection: 'row', gap: 8, padding: 10 },
  errorText: { flex: 1, fontSize: 12, lineHeight: 17 },
  finishedBody: { fontSize: 15, lineHeight: 21, maxWidth: 330, textAlign: 'center' },
  finishedKicker: { fontSize: 13, lineHeight: 18 },
  finishedTitle: { fontSize: 49, lineHeight: 50, textAlign: 'center' },
  ...
  guideButton: { alignItems: 'center', borderRadius: 15, borderWidth: 1, flexDirection: 'row', gap: 10, minHeight: 58, paddingHorizontal: 12 },
  guideCopy: { flex: 1, gap: 2 },
  guideLabel: { fontSize: 12, lineHeight: 16 },
  guideSheet: { borderTopLeftRadius: 24, borderTopRightRadius: 24, borderTopWidth: 4, gap: 14, paddingBottom: 34, paddingHorizontal: 20, paddingTop: 20 },
  guideValue: { fontSize: 13 },
  hint: { alignItems: 'center', borderRadius: 13, borderWidth: 1, flexDirection: 'row', gap: 11, padding: 11 },
  hintButton: { alignItems: 'center', borderRadius: 15, justifyContent: 'center', minHeight: 58, paddingHorizontal: 16 },
  hintButtonText: { color: '#171309', fontSize: 16, lineHeight: 20, textAlign: 'center' },
  hintIndex: { fontSize: 25 },
  hintText: { flex: 1, fontSize: 14, lineHeight: 19 },
  instruction: { flex: 1, fontSize: 15, lineHeight: 21 },
  instructionBand: { alignItems: 'flex-start', borderRadius: 15, borderWidth: 1, flexDirection: 'row', gap: 10, padding: 13 },
  ...
  loadingTitle: { fontSize: 32, lineHeight: 34, textAlign: 'center' },
  modalBackdrop: { backgroundColor: 'rgba(0,0,0,0.72)', flex: 1, justifyContent: 'flex-end' },
  noHints: { fontSize: 12, lineHeight: 17, paddingVertical: 12, textAlign: 'center' },
  page: { flexGrow: 1, paddingBottom: 35, paddingHorizontal: 20, paddingTop: 20 },
  pageWithDock: { paddingBottom: 116 },
  penalty: { alignItems: 'center', borderRadius: 13, flexDirection: 'row', gap: 8, justifyContent: 'center', minHeight: 47, marginTop: 13 },
  ...
  reconnectButton: { borderRadius: 13, borderWidth: 1, marginTop: 16, paddingHorizontal: 18, paddingVertical: 11 },
  ...
  stageIndex: { alignItems: 'center', borderRadius: 13, borderWidth: 1, height: 43, justifyContent: 'center', width: 48 },
  stageIndexText: { fontSize: 20 },
  stageKicker: { fontSize: 13, lineHeight: 18 },
  stageTitle: { fontSize: 38, lineHeight: 40 },
  teamIdentity: { alignItems: 'center', flexDirection: 'row', gap: 9 },
  teamLabel: { fontSize: 11, lineHeight: 14 },
