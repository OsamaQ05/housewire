        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
          <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.hero}>
              ...
              <Text style={[styles.kicker, { color: MINT, fontFamily: theme.typography.families.monoMedium }]}>ANOTHER PHONE IS HOLDING THE START CLOCK</Text>
              <Text style={[styles.title, { color: theme.colors.text, fontFamily: theme.typography.families.displayHeavy }]}>SCAN INTO THE GRID.</Text>
              <Text style={[styles.subtitle, { color: theme.colors.muted, fontFamily: theme.typography.families.body }]}>One scan carries the room code and host address.</Text>
            </View>
            ...
            <Pressable accessibilityRole="button" onPress={() => void openScanner()} style={({ pressed }) => [styles.scanButton, { borderColor: MINT, backgroundColor: theme.colors.surface }, pressed && styles.pressed]}>
              ...
              <View style={styles.scanCopy}>
                <Text style={[styles.scanTitle, { color: theme.colors.text, fontFamily: theme.typography.families.displayHeavy }]}>{code.length === 5 ? 'INVITE LOADED' : 'SCAN HOST QR'}</Text>
                <Text style={[styles.scanMeta, { color: theme.colors.faint, fontFamily: theme.typography.families.mono }]}>{code.length === 5 ? code : 'CAMERA OPENS ONLY WHILE SCANNING'}</Text>
              </View>
            ...
            <Pressable accessibilityRole="button" disabled={joining} onPress={() => void join()} style={({ pressed }) => [styles.joinButton, { backgroundColor: MINT }, joining && styles.disabled, pressed && styles.pressed]}>
              <Text style={[styles.joinText, { fontFamily: theme.typography.families.displayHeavy }]}>{joining ? 'FINDING START CLOCK…' : 'JOIN THE RACE'}</Text>
...
function Field({ autoCapitalize = 'words', label, maxLength = 120, onChange, placeholder, value }: { autoCapitalize?: 'none' | 'words' | 'characters'; label: string; maxLength?: number; onChange(value: string): void; placeholder: string; value: string }) {
  const { theme } = useHousewireTheme();
  return <View style={styles.field}><Text style={[styles.fieldLabel, { color: theme.colors.faint, fontFamily: theme.typography.families.monoMedium }]}>{label.toUpperCase()}</Text><TextInput autoCapitalize={autoCapitalize} autoCorrect={false} maxLength={maxLength} onChangeText={onChange} placeholder={placeholder} placeholderTextColor={theme.colors.faint} style={[styles.input, { backgroundColor: theme.colors.surface, borderColor: theme.colors.draft, color: theme.colors.text, fontFamily: theme.typography.families.bodyMedium }]} value={value} /></View>;
}
...
const styles = StyleSheet.create({
  ...
  error: { alignItems: 'center', borderLeftWidth: 3, flexDirection: 'row', gap: 8, padding: 10 },
  ...
  input: { borderWidth: 1, fontSize: 16, minHeight: 56, paddingHorizontal: 13 },
  joinButton: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', minHeight: 66, paddingHorizontal: 17 },
  joinText: { color: '#08100F', fontSize: 20, letterSpacing: 0.4 },
  kicker: { fontSize: 7, letterSpacing: 1.2, marginTop: 13, textAlign: 'center' },
  manualFields: { gap: 12 },
  ...
  scanButton: { alignItems: 'center', borderLeftWidth: 4, borderWidth: 1, flexDirection: 'row', gap: 12, minHeight: 78, padding: 12 },
  scanCopy: { flex: 1, gap: 2 },
  scanFrame: { borderColor: MINT, borderWidth: 3, height: 250, position: 'absolute', width: 250 },
  scanMeta: { fontSize: 7, letterSpacing: 1 },
  ...
  scannerText: { bottom: 90, color: '#F4E8CF', fontSize: 11, letterSpacing: 1.4, position: 'absolute' },
  scanTitle: { fontSize: 22, lineHeight: 23 },
  subtitle: { fontSize: 14, lineHeight: 20, marginTop: 7, textAlign: 'center' },
  title: { fontSize: 43, lineHeight: 41, marginTop: 7, textAlign: 'center' },
});
