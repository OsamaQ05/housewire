...
function SequenceChallenge({ accent, onSubmit, stage }: ExtractProps<'sequence-cipher'>) {
  ...
  return (
    <View style={styles.challenge}>
      {stage.challenge.station !== 'FULL' ? <RoleStation accent={accent} icon={plates ? 'book-outline' : 'hand-left-outline'} label={plates ? 'RIDDLE READER' : 'SWITCHBOARD RUNNER'} text={plates ? 'Read each clue aloud, in order. Do not say the hidden answer.' : 'Listen, solve, and hit the matching object. Your teammate cannot touch this board.'} /> : null}
      ...
      {wheel ? <View style={[styles.wheel, { borderColor: theme.colors.draft }]}> 
        <View style={styles.wheelHeader}>
          <Text style={[styles.microLabel, { color: theme.colors.faint, fontFamily: theme.typography.families.monoMedium }]}>OBJECT SWITCHBOARD</Text>
      ...
      {canEnter ? (
        <View style={styles.answerConsole}>
          <Text style={[styles.microLabel, { color: theme.colors.faint, fontFamily: theme.typography.families.monoMedium }]}>YOUR FOUR ANSWERS</Text>
          ...
          <View style={styles.actionRow}>
            ...
            <View style={styles.submitGrow}><SubmitBar accent={accent} disabled={code.length !== 4} label="ROUTE ANSWERS" onPress={submit} /></View>
          </View>
        </View>
  ...
  return (
    <View style={styles.challenge}>
      ...
      <View accessibilityLabel={`Flight path ${tiltSequence.map(motionMoveLabel).join(', ')}, then ${faceLabel}`} accessible style={[styles.flightPlan, { borderColor: accent }]}> 
        <Text style={[styles.orientationLabel, { color: theme.colors.faint, fontFamily: theme.typography.families.monoMedium }]}>SECRET FLIGHT PLAN · CALL IN ORDER</Text>
...
function GroundActionStation({ accent, challenge, onSubmit }: {
  ...
  return (
    <View style={styles.challenge}>
      {challenge.station === 'GROUND' ? <RoleStation accent={accent} icon="phone-portrait-outline" label="BLIND PILOT" text="You cannot see the route. Follow each direction your teammate calls, return to center, then land as ordered." /> : <RoleStation accent={accent} icon="navigate-circle-outline" label="SOLO FLIGHT" text="Fly the three shown movements in order. Return to center between moves, then land the phone." />}
      ...
      {!ground.active ? (
        <Pressable accessibilityLabel="Arm phone motion sensor" accessibilityRole="button" onPress={() => void ground.start()} style={({ pressed }) => [styles.armButton, { backgroundColor: accent }, pressed && styles.pressed]}>
          ...
          <Text style={[styles.armText, { fontFamily: theme.typography.families.displayHeavy }]}>ARM FLIGHT SENSOR</Text>
        </Pressable>
      ) : null}
  ...
  return (
    <View accessibilityLabel={`${label}. ${text}`} accessible style={[styles.roleStation, { backgroundColor: theme.colors.surface, borderColor: accent }]}> 
      ...
      <View style={styles.roleCopy}>
        <Text style={[styles.roleLabel, { color: accent, fontFamily: theme.typography.families.monoMedium }]}>{label}</Text>
...
const styles = StyleSheet.create({
  ...
  answerSocket: { alignItems: 'center', borderWidth: 1, flex: 1, height: 57, justifyContent: 'center' },
  armButton: { alignItems: 'center', flexDirection: 'row', gap: 8, justifyContent: 'center', minHeight: 61 },
  armText: { color: '#08100F', fontSize: 20, letterSpacing: 0.3 },
  challenge: { gap: 15 },
  ...
  echoPad: { alignItems: 'center', borderWidth: 2, gap: 5, justifyContent: 'center', minHeight: 190 },
  ...
  errorLine: { alignItems: 'center', borderLeftWidth: 3, flexDirection: 'row', gap: 8, paddingHorizontal: 10, paddingVertical: 9 },
  ...
  flightPlan: { borderTopWidth: 3, gap: 17, padding: 16 },
  fragment: { borderLeftWidth: 4, borderWidth: 1, gap: 9, padding: 13 },
  ...
  holdButton: { alignItems: 'center', borderWidth: 1, justifyContent: 'center', minHeight: 58 },
  holdText: { fontSize: 18 },
  knockChoice: { alignItems: 'center', borderWidth: 1, flex: 1, flexDirection: 'row', gap: 8, minHeight: 47, paddingHorizontal: 10 },
  ...
  levelMeta: { fontSize: 8, letterSpacing: 1.1 },
  levelPlate: { alignItems: 'center', borderWidth: 1, gap: 10, padding: 17 },
  ...
  listenButton: { alignItems: 'center', alignSelf: 'stretch', borderWidth: 1, flexDirection: 'row', gap: 8, justifyContent: 'center', minHeight: 52 },
  listenPanel: { alignItems: 'center', borderLeftWidth: 4, borderWidth: 1, gap: 14, padding: 15 },
  listenText: { fontSize: 14 },
  manualDirection: { alignItems: 'center', borderWidth: 1, height: 52, justifyContent: 'center', width: '22%' },
  manualDirections: { flexDirection: 'row', gap: 7, justifyContent: 'space-between' },
  microLabel: { fontSize: 7, letterSpacing: 1.2 },
  moveRail: { alignItems: 'center', flexDirection: 'row', gap: 7, justifyContent: 'center' },
  moveSlot: { alignItems: 'center', borderWidth: 1, height: 47, justifyContent: 'center', width: 47 },
  moveSlotText: { fontSize: 20 },
  objectName: { fontSize: 9, letterSpacing: 0.8 },
  objectSwitch: { alignItems: 'center', borderWidth: 1, gap: 7, minHeight: 82, padding: 9, width: '31%' },
  orientationFace: { fontSize: 39, lineHeight: 42, textAlign: 'center' },
  orientationLabel: { fontSize: 9, letterSpacing: 1.3 },
  orientationPlate: { alignItems: 'center', borderWidth: 2, gap: 10, minHeight: 210, justifyContent: 'center', padding: 20 },
  plate: { alignItems: 'center', borderWidth: 1, flex: 1, gap: 5, minHeight: 112, justifyContent: 'center', paddingVertical: 8 },
  ...
  roleIcon: { alignItems: 'center', borderWidth: 1, height: 43, justifyContent: 'center', width: 43 },
  roleLabel: { fontSize: 9, letterSpacing: 1.2 },
  roleStation: { alignItems: 'center', borderLeftWidth: 4, borderWidth: 1, flexDirection: 'row', gap: 11, padding: 12 },
  roleText: { fontSize: 13, lineHeight: 18 },
  riddleCard: { alignItems: 'center', borderLeftWidth: 3, borderWidth: 1, flexDirection: 'row', gap: 11, minHeight: 82, padding: 11 },
  riddleIndex: { alignItems: 'center', height: 39, justifyContent: 'center', width: 39 },
  riddleIndexText: { color: '#171309', fontSize: 22 },
  riddleStack: { gap: 7 },
  riddleText: { flex: 1, fontSize: 16, lineHeight: 21 },
  submit: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', minHeight: 58, paddingHorizontal: 15 },
  submitGrow: { flex: 1 },
  submitText: { color: '#08100F', fontSize: 19, letterSpacing: 0.4 },
  teammateNote: { fontSize: 13, lineHeight: 18 },
  undoButton: { alignItems: 'center', borderWidth: 1, flexDirection: 'row', gap: 6, justifyContent: 'center', minHeight: 58, paddingHorizontal: 12 },
  ...
  wheelEntry: { alignItems: 'center', borderWidth: 1, flexDirection: 'row', gap: 5, minHeight: 48, paddingHorizontal: 8, width: '31%' },
  wheelHeader: { gap: 3 },
  ...
  weaveArrowButton: { alignItems: 'center', borderWidth: 1, flex: 1, justifyContent: 'center', minHeight: 42 },
  ...
  weaveStep: { alignItems: 'center', borderWidth: 1, flex: 1, height: 49, justifyContent: 'center' },
